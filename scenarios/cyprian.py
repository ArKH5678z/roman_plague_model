...
# Build network - weakened from Antonine losses
# Remove or reduce capacity of some nodes to simulate depopulation
G = nx.Graph()
...
# Simulate Antonine network damage
# Remove 10% of edges randomly to represent depopulation and abandoned routes
np.random.seed(42)
all_edges = list(G.edges())
edges_to_remove = np.random.choice(len(all_edges), 
                                    size=int(len(all_edges) * 0.10), 
                                    replace=False)
for idx in edges_to_remove:
    G.remove_edge(*all_edges[idx])

print(f"Network after Antonine damage: {G.number_of_edges()} edges (was 560)")

# Cyprian plague parameters
# Started in Ethiopia/Egypt 249 AD
# Unknown pathogen - faster and more lethal than Antonine
START_YEAR = 249
START_CITY = 'Alexandria'
DAYS = 365

# Higher beta, lower gamma than Antonine
beta_base = 0.60
gamma_base = 0.02

# PID parameters - same controller, harder conditions
Kp = 0.8
Ki = 0.01
Kd = 0.1
setpoint = 15
...
if __name__ == "__main__":
    print("\nRunning Cyprian Plague simulation...")
    print(f"Start: {START_CITY}, {START_YEAR} AD")
    print(f"Duration: {DAYS} days")

    for use_pid in [True, False]:
        S, I, R, beta_hist, final_states = run_cyprian(use_pid=use_pid)
        ...
        filename = f'outputs/cyprian_{"pid" if use_pid else "nopid"}_results.json'
